...
**Review follow-up**: PR #45 review tightened this further: the GitHub release check now requires both `ContainerBar.zip` and `ContainerBar.dmg`, and the appcast check verifies `sparkle:edSignature` plus `length` against the uploaded GitHub release zip rather than a local `dist` archive. The Python suite now has 46 standalone tests.
